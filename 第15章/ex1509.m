...
[fc_Nelder,fo_Nelder,co_Nelder] = f1509(xo_Nelder) %Nelder方法结果
[xo_s,fo_s] = fminsearch(f,x0) %MATLAB 内置函数fminsearch()
[fc_s,fo_s,co_s] = f1509(xo_s) %相应的结果
...
[fc_steep,fo_steep,co_steep] = f1509(xo_steep) %相应结果
[xo_u,fo_u] = fminunc(f,x0); % MATLAB 内置函数fminunc()
[fc_u,fo_u,co_u] = f1509(xo_u) %相应结果